...
PyGuitarPro is a package to read, write and manipulate GP3, GP4, GP5, and GP8 files. Initially, PyGuitarPro is a Python port of [AlphaTab](http://www.alphatab.net/) which is a Haxe port of [TuxGuitar](http://tuxguitar.herac.com.ar/).

**Note:** Support for GP8 files was added later and is still experimental. GP8 files use a different format compared to previous versions, utilizing an XML-based structure known as GPIF (Guitar Pro Intermediate Format). This format is more flexible and allows for more detailed musical notation, but it also means that some features may not be fully supported yet. GP8 files typically have the `.gp` extension.
...
To anyone wanting to create their best guitar tablature editor in Python, this package will be a good thing to start with.
